...
<html>
<head>
    <title>Laporan Anggota - {{ $ekskulNama }}</title>
    <style>
        body {
    ...
    <div class="meta">
        <table>
            <tr>
                ...
                <td style="width: 3%;">:</td>
                <td>Daftar Anggota Ekstrakurikuler</td>
        ...
        <tbody>
            @forelse($anggota as $index => $a)
                <tr>
                    ...
                    <td>{{ $a->tanggal_bergabung ? $a->tanggal_bergabung->format('Y-m-d') : '-' }}</td>
                    <td class="text-center">{{ ucfirst($a->status) }}</td>
                </tr>
            @empty
                <tr>
                    <td colspan="7" class="text-center" style="padding: 15px; color: #888;">Belum ada anggota aktif terdaftar.</td>
                </tr>
            @endforelse
        </tbody>
    </table>
